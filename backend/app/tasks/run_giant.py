...
from app.crawlers.giant import GiantCrawler
...
def run(limit: int | None = None) -> dict[str, int]:
    ...
    with SessionLocal() as db:
        source_site = ensure_giant_source_site(db, crawler)
        job = CrawlerJob(
        ...
        try:
            for product in products:
                try:
                    ...
                    crawler_repo.save_snapshot(
                        source_site=source_site,
                        url=crawler.detail_api_url.format(bike_id=product.source_product_id),
                    ...
                    db.rollback()
                    stats["failed"] += 1
                    print(f"[giant] failed product={product.source_product_id}: {exc}")
                else:
                    db.commit()
...
def ensure_giant_source_site(db, crawler: GiantCrawler) -> SourceSite:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Import Giant road bike products.")
