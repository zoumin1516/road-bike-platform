...
from app.crawlers.specialized import SpecializedCrawler
...
def run(limit: int | None = None) -> dict[str, int]:
    ...
    with SessionLocal() as db:
        source_site = ensure_specialized_source_site(db, crawler)
        job = CrawlerJob(
        ...
        try:
            for product in products:
                try:
                    ...
                    crawler_repo.save_snapshot(
                        source_site=source_site,
                        url=crawler.product_detail_url.format(
                            product_code=product.source_product_id
                        ),
                    ...
                    db.rollback()
                    stats["failed"] += 1
                    print(f"[specialized] failed product={product.source_product_id}: {exc}")
                else:
                    db.commit()
...
def ensure_specialized_source_site(db, crawler: SpecializedCrawler) -> SourceSite:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Import Specialized China road bike products.")
